...
func TestGetBackupDBCountMetrics(t *testing.T) {
	type args struct {
		config              string
	...
	tests := []struct {
		name               string
		args               args
		mockTestDataBackup mockStruct
	}{
		// All metrics exist and all labels are corrected.
		// pgBackrest version = latest.
		{
			"getBackupMetricsDatabases",
			args{
				"",
				"",
				templateStanza(
				...
				templateStanza(
					...
					annotation{"testkey": "testvalue"}).Backup,
				setUpMetricValue,
				`# HELP pgbackrest_backup_databases Number of databases in backup.
# TYPE pgbackrest_backup_databases gauge
pgbackrest_backup_databases{backup_name="20210607-092423F",backup_type="full",block_incr="y",database_id="1",repo_key="1",stanza="demo"} 0
`,
			},
			mockStruct{
		...
		{
			"getBackupMetricsDatabasesAbsent",
			args{
				"",
				"",
				templateStanzaDBsAbsent(
				...
				`# HELP pgbackrest_backup_databases Number of databases in backup.
# TYPE pgbackrest_backup_databases gauge
pgbackrest_backup_databases{backup_name="20210607-092423F",backup_type="full",block_incr="n",database_id="1",repo_key="1",stanza="demo"} 0
`,
			},
			mockStruct{
				"",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resetBackupMetrics()
			mockDataBackup = tt.mockTestDataBackup
			execCommand = fakeExecCommand
			defer func() { execCommand = exec.Command }()
			lc := log.NewNopLogger()
			getBackupDBCountMetrics(1, tt.args.config, tt.args.configIncludePath, tt.args.stanzaName, tt.args.backupData, tt.args.setUpMetricValueFun, lc)
			reg := prometheus.NewRegistry()
			reg.MustRegister(
